...
function parseSources(str){
  ...
  return String(str).toLowerCase().split(/[,;/|+]+/).map(s => {
    const clean = s.trim();
    // Normalizziamo le stringhe per evitare duplicati o errori
...
const LibraryItem = memo(({ 
  ...
  isArchiveView, 
  onToggleFocus, 
  onToggleQueue,    // <--- PROP FONDAMENTALE
  onMarkPurchased, 
  onArchive, 
  onEdit, 
  onReExperience, 
  ...
  // FIX: Controllo Coda Case-Insensitive per sicurezza
...
export default function App(){
  ...
  /* --- FIX: FUNZIONE CODA CORRETTA --- */
  const toggleQueue = useCallback(async (it) => {
    // 1. Legge le fonti attuali
    const currentSources = it.sourcesArr || [];
    
    // 2. Controllo robusto (case-insensitive) per vedere se è già in coda
    const isInQueue = currentSources.some(s => s.toLowerCase() === "coda");
    
    // LOGICA DI CONTROLLO SLOT (Se vogliamo AGGIUNGERE)
    if (!isInQueue) {
      // Conta quanti elementi hanno "Coda" nelle loro fonti
    ...
    if (isInQueue) {
      // --- RIMOZIONE ---
      // Filtra via "Coda" (case-insensitive)
    ...
    if (!error) {
       ...
       showToast("Errore DB: " + error.message, "error");
    }
  }, [items, showToast]);
  ...
  return (
    <div className="app">
      ...
      {/* ===== Ricerca Zen "Cool Gray" (STICKY & Z-INDEX ALTO) ===== */}
      <section className="card" style={{
        ...
        position: 'sticky', top: 10, zIndex: 100 // <--- FISSATA IN ALTO
      }}>
                   ...
                   {items.filter(i => (i.sourcesArr||[]).some(s => s.toLowerCase() === 'coda')).length === 0 ? <p style={{textAlign:'center', opacity:0.6}}>La coda è vuota.</p> : (
                     items.filter(i => (i.sourcesArr||[]).some(s => s.toLowerCase() === 'coda')).map(w => (
                       <div key={w.id} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom:8, paddingBottom:8, borderBottom:'1px dashed rgba(0,0,0,0.1)'}}>
                          <div style={{color:'#44337a', fontSize:'0.95em'}}>{TYPE_ICONS[w.kind]} {w.title} {w.mood && <span style={{fontSize:'0.7em', marginLeft:6, opacity:0.7, border:'1px solid #d6bc9b', borderRadius:4, padding:'0 4px'}}>{w.mood}</span>}</div>
      ...
      {/* ===== TUTTI I MODALI (Con stile FIXED inline per sicurezza) ===== */}
          ...
          <div className="card" style={{maxWidth:600, width:"94%", maxHeight:"90vh", overflowY:"auto", padding:"20px 24px", borderRadius: 20, backgroundColor:'#FDF8F2'}} onClick={e => e.stopPropagation()}>
            ...
            {statsView === 'totale' && (
              <div style={{animation:'fadeIn 0.3s'}}>
                ...
                <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 12px', border:'1px solid #bee3f8', backgroundColor:'#ebf8ff', borderRadius:12, color:'#2b6cb0'}}><span>🛒 Wishlist (Wishlist)</span><strong>{stats.bySource[0]?.n || 0}</strong></div>
              </div>
            )}
