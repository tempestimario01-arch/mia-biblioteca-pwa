...
/* =========================================
   1. CONSTANTS AND ZEN CONFIGURATION
   ========================================= */

// --- ZEN RULES ---
const MAX_ACTIVE = 3;  // Max items In Progress
const MAX_QUEUE = 3;   // Max items In Queue (Total visual: 6)
const HEAVY_MOODS = ["Apprendimento", "Impegnativo"]; // Moods requiring high energy
...
// Robust parser to intercept Queue
...
const LibraryItem = memo(({ 
  ...
  isArchiveView, 
  onToggleFocus, 
  onToggleQueue, // ADDED QUEUE LOGIC
  onMarkPurchased, 
  onArchive, 
  onEdit, 
  onReExperience, 
  ...
  // VISUAL LOGIC: Green (Active), Purple (Queue), Default
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', gap: 12, marginTop: 4, paddingTop: 12, borderTop: '1px solid #f0f4f8', flexWrap: 'wrap' }}>
        ...
        {(!it.finished_at && !isArchived) && (
          <>
            ...
            {/* QUEUE BUTTON (Only if not active) */}
            {!it.is_next && (
...
export default function App(){
  ...
  // --- ZEN FETCH: Download BOTH Active AND Queue items ---
  const fetchPinnedItems = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      // SORTING: Active (Green) First, then Queue (Purple)
  ...
  // --- ZEN LOGIC: QUEUE TOGGLE (With Limit) ---
  const toggleQueue = useCallback(async (it) => {
    ...
    // Zen Rule: Max 3 in Queue
    if (!isInQueue) {
      const queueCount = pinnedItems.filter(p => !p.is_next && (p.sourcesArr||[]).some(s=>s.toLowerCase()==='coda')).length;
      if (queueCount >= MAX_QUEUE) { showToast(`✋ Coda Zen Piena! Max ${MAX_QUEUE} elementi.`, "error"); return; }
    }
  ...
  // --- ZEN LOGIC: FOCUS TOGGLE (With Cognitive Rules) ---
  const toggleFocus = useCallback(async (it) => {
    if (!it.is_next) {
      // Rule 1: Max 3 Active
      const activeCount = pinnedItems.filter(p => p.is_next).length;
      if (activeCount >= MAX_ACTIVE) { showToast(`🧠 Massimo ${MAX_ACTIVE} progetti attivi!`, "error"); return; }

      // Rule 2: Cognitive Load (Max 1 Heavy)
      const isHeavy = HEAVY_MOODS.includes(it.mood);
      if (isHeavy) {
        const hasHeavyActive = pinnedItems.some(p => p.is_next && HEAVY_MOODS.includes(p.mood));
        if (hasHeavyActive) { showToast("✋ Hai già un progetto impegnativo attivo. Finiscilo prima!", "error"); return; }
      }
    } 
    else {
  ...
  return (
    <div className="app">
      ...
      {/* PIANO DI LETTURA ZEN (LISTA UNICA 3+3) */}
      {!isSearchActive && !loading && pinnedItems.length > 0 && (
        <section className="card" style={{marginTop: 12, marginBottom:12, padding:'0', overflow:'hidden', border:'1px solid #e2e8f0'}}>
          <div style={{padding:'10px 16px', backgroundColor:'#f7fafc', borderBottom:'1px solid #edf2f7', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
             <span style={{fontWeight:'bold', color:'#2d3748'}}>Il tuo Percorso</span>
             <span style={{fontSize:'0.8em', color:'#718096'}}>{pinnedItems.filter(x=>x.is_next).length} in corso • {pinnedItems.filter(x=>!x.is_next).length} in coda</span>
          </div>
          <div style={{padding:'8px 12px'}}>
            {pinnedItems.map((p) => {
              const isActive = p.is_next;
              const isHeavy = HEAVY_MOODS.includes(p.mood);
              return (
                <div key={p.id} style={{
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    padding: '10px', marginBottom: 8, borderRadius: 10,
                    backgroundColor: isActive ? '#f0fff4' : '#fff',
                    borderLeft: isActive ? '4px solid #38a169' : '4px solid #805ad5',
                    border: isActive ? '1px solid #c6f6d5' : '1px dashed #cbd5e0',
                    borderLeftWidth: 4
                }}>
                  <div style={{flex: 1}}>
                    <div style={{display:'flex', alignItems:'center', gap:6}}>
                       <span style={{fontSize:'1.2em'}}>{isActive ? '🔥' : '⏳'}</span>
                       <div style={{fontWeight:'600', color: isActive ? '#22543d' : '#4a5568'}}>
                         {TYPE_ICONS[p.kind]} {p.title}
                       </div>
                    </div>
                    <div style={{display:'flex', alignItems:'center', gap:8, marginTop:4, marginLeft: 28}}>
                        <span style={{fontSize:'0.85em', opacity:0.8}}>{p.creator}</span>
                        {p.mood && (
                          <span style={{
                            fontSize:'0.7em', padding:'1px 5px', borderRadius:4, 
                            backgroundColor: isHeavy ? '#fff5f5' : '#ebf8ff',
                            color: isHeavy ? '#c53030' : '#2b6cb0',
                            border: `1px solid ${isHeavy ? '#feb2b2' : '#bee3f8'}`
                          }}>
                            {p.mood}
                          </span>
                        )}
                  ...
                  <div style={{display:'flex', gap:6}}>
                    {isActive ? (
                       <button className="ghost" onClick={() => openArchiveModal(p)} title="Finito!" style={{fontSize:'1.2em', padding:'6px', backgroundColor:'white', border: '1px solid #c6f6d5', borderRadius: '8px'}}>📦</button>
                    ) : (
                       <button className="ghost" onClick={() => toggleFocus(p)} title="Inizia Ora" style={{fontSize:'1.2em', padding:'6px', backgroundColor:'white', border: '1px solid #d6bc9b', borderRadius: '8px'}}>🚀</button>
                    )}
                  </div>
                </div>
              );
      ...
      {/* Modale Aggiunta (COPIATO DAL VECCHIO) */}
      {addModalOpen && (
      ...
      {/* Modale Avanzato (COPIATO DAL VECCHIO) */}
      {advOpen && (
      ...
      {/* Modale Statistiche (COPIATO DAL VECCHIO) */}
      {statsModalOpen && (
      ...
      {/* Modale Archivia (COPIATO DAL VECCHIO) */}
      {archModal && (
      ...
      {/* Modale Modifica (COPIATO DAL VECCHIO) */}
      {editState && (
